    ...
    for (TR::ParameterSymbol *paramCursor = paramIterator.getFirst(); paramCursor != NULL;
        ...
        switch (paramCursor->getDataType()) {
            ...
            case TR::Double: {
                ...
                // On 64-bit XPLINK floating point arguments leave "holes" in the GPR linkage registers, but not vice
                // versa
                numGPRArgs++;
                numFPRArgs++;
                break;
...
TR::Instruction *TR::S390zOSSystemLinkage::genCallNOPAndDescriptor(TR::Instruction *cursor, TR::Node *node,
    TR::Node *callNode, TR_XPLinkCallTypes callType)
{
...
TR::Instruction *TR::S390zOSSystemLinkage::spillGPRsInPrologue(TR::Node *node, TR::Instruction *cursor)
{
    ...
    switch (frameType) {
        ...
        case TR_XPLinkIntermediateFrame:
            if (needAddTempReg) {
                ...
                if (saveTempReg || needAddTempReg) {
                    cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, spReg,
                        cursor); // LR R0,R4
                }
                if (saveTempReg) {
                    gpr3ParmRef
                        = generateS390MemoryReference(spReg, gpr3ParmOffset, cg()); // used for temporary register
                    cursor = generateRXInstruction(cg(), TR::InstOpCode::getStoreOpCode(), node, gpr3Real, gpr3ParmRef,
                        cursor); // ST R3,parmRef(R4)
                }

                _stackPointerUpdateLabel = generateLabelSymbol(cg());
                cursor
                    = generateS390LabelInstruction(cg(), TR::InstOpCode::label, node, _stackPointerUpdateLabel, cursor);

                cursor = addImmediateToRealRegister(spReg, (stackFrameSize) * -1, gpr3Real, node,
                    cursor); // R4 <- R4 - stack size

                if (saveTempReg) {
                    cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr3Real, gpr0Real,
                        cursor);
                    gpr3ParmRef = generateS390MemoryReference(gpr3Real, gpr3ParmOffset, cg());
                    cursor = generateRXInstruction(cg(), TR::InstOpCode::getLoadOpCode(), node, gpr3Real, gpr3ParmRef,
                        cursor);
                }
        ...
        case TR_XPLinkStackCheckFrame:
            ...
            if (!needAddTempReg) {
                cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), node, gpr0Real, gpr3Real,
                    cursor); // LR R0,R3
            }
...
TR::S390zOSSystemLinkage::XPLINKCallDescriptorRelocation::XPLINKCallDescriptorRelocation(TR::Instruction *nop,
