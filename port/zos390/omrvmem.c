...
#pragma linkage(j9allocate_1M_pageable_pages_below_bar,OS_NOSTACK)
void *j9allocate_1M_pageable_pages_below_bar(long numBytes, int subpool);

/* omrvmem_support_below_bar_[31|64].s */
#pragma linkage(j9allocate_4K_pages_below_bar,OS_NOSTACK)
void *j9allocate_4K_pages_below_bar(long numBytes, int subpool);

/* omrvmem_support_below_bar_[31|64].s */
#pragma linkage(j9free_memory_below_bar,OS_NOSTACK)
int j9free_memory_below_bar(void *address, long length, int subpool);

#if 0
/* Enable this to use hand coded assembler routine instead of the one generated by Metal-C. */
#pragma linkage (ALPPBB,OS)
#pragma map (j9allocate_1M_pageable_pages_below_bar, "ALPPBB")
void *j9allocate_1M_pageable_pages_below_bar(size_t length, int subpool);
#endif
...
/* omrget_two_to_thirtytwo_supported.s */
#pragma linkage (GETTTT,OS)
#pragma map (Get_Two_to_Thirtytwo_Supported,"GETTTT")
uintptr_t Get_Two_to_Thirtytwo_Supported();
...
#if defined(OMR_ENV_DATA64)
					result = j9discard_data((void *)address, byteAmount >> ZOS_REAL_FRAME_SIZE_SHIFT);
#else
					result = Pgser_Release((void *)address, byteAmount);
#endif /* defined(OMR_ENV_DATA64) */
					break;
...
omrvmem_free_memory(struct OMRPortLibrary *portLibrary, void *address, uintptr_t byteAmount, struct J9PortVmemIdentifier *identifier)
{
	...
	if (OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY == identifier->allocator) {
		...
		rc = j9free_memory_below_bar(address, identifier->size, OMRPORT_VMEM_SUBPOOL);
		omrmem_categories_decrement_counters(identifier->category, identifier->size);
#if defined(OMR_ENV_DATA64)
		...
		rc = j9free_memory_above_bar(address, ttkn);
		omrmem_categories_decrement_counters(identifier->category, identifier->size);
...
default_pageSize_reserve_low_memory(struct OMRPortLibrary *portLibrary, uintptr_t byteAmount, struct J9PortVmemIdentifier *identifier, uintptr_t mode, OMRMemCategory *category)
{
	...
	if (J9_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
		...
		ptr = j9allocate_4K_pages_below_bar(allocSize, OMRPORT_VMEM_SUBPOOL);
...
/*
 * Use STORAGE OBTAIN macro via j9allocate_1M_pageable_pages_below_bar() to allocate memory below 2GB bar.
...
reserve1MPageableBelowBar(struct OMRPortLibrary *portLibrary, J9PortVmemIdentifier *identifier, uintptr_t mode, uintptr_t byteAmount, OMRMemCategory *category)
{
	...
	LP_DEBUG_PRINTF1("\t reserve1MPageableBelowBar calling j9allocate_1M_pageable_pages_below_bar(0x%x bytes)\n", byteAmount);

	ptr = j9allocate_1M_pageable_pages_below_bar(byteAmount, OMRPORT_VMEM_SUBPOOL);

	LP_DEBUG_PRINTF2("\t j9allocate_1M_pageable_pages_below_bar(0x%x bytes) returned 0x%zx\n", byteAmount, ptr);
...
#if defined(OMR_ENV_DATA64)
/*
 * Use IARV64 macro via j9allocate_xxx family of routines to allocate memory above 2GB bar.
...
{
	/* The maximum amount of memory that can be allocated when use2To32GArea now is changed to 64GB (64 - 2 = 62) */
#define MAX_2TO32G_MEMORY_IN_MB ((uintptr_t)62*1024)	/* unit is MB i.e. 62*1024 MB = 62 GB */
	...
	BOOLEAN use2To32GArea = (OMRPORT_VMEM_ZOS_USE2TO32G_AREA == (OMRPORT_VMEM_ZOS_USE2TO32G_AREA & options));

	Trc_PRT_vmem_reservePagesAboveBar_Entry(byteAmount, pageSize, pageFlags, options, use2To32GArea);

	LP_DEBUG_PRINTF5("\t reservePagesAboveBar byteAmount=0x%zx, pageSize=x%zx, pageFlags=0x%zx, useStrictPageSize=0x%x, use2To32G=0x%x\n", \
					 byteAmount, pageSize, pageFlags, useStrictPageSize, use2To32GArea);
	...
	if ((FOUR_K == pageSize) &&
		...
		LP_DEBUG_PRINTF1("\t reservePagesAboveBar calling j9allocate_4K_pages_in_2to32G_area(0x%zx)\n", \
						 numSegments);
		...
		ptr = j9allocate_4K_pages_in_2to32G_area(numSegments, ttkn);

		LP_DEBUG_PRINTF2("\t j9allocate_4K_pages_in_2to32G_area(0x%zx) returned 0x%zx\n", \
						 numSegments, ptr);
		...
		if (0 != PPG_vmem_pageSize[index]) {
			if ((index > 0) &&
				...
				LP_DEBUG_PRINTF2("\t reservePagesAboveBar calling j9allocate_2G_pages(0x%zx, 0x%x)\n", \
								 numUnits, use2To32GArea);
				Trc_PRT_vmem_reservePagesAboveBar_allocate_large_2G_pages(numUnits, use2To32GArea);
				ptr = j9allocate_2G_pages(numUnits, use2To32GArea, ttkn);

				LP_DEBUG_PRINTF3("\t j9allocate_2G_pages(0x%zx, 0x%x) returned 0x%zx\n", \
								 numUnits, use2To32GArea, ptr);
				if (NULL == ptr) {
			...
			if ((NULL == ptr) &&
				...
				LP_DEBUG_PRINTF2("\t reservePagesAboveBar calling j9allocate_1M_fixed_pages(0x%zx, 0x%x)\n", \
								 numSegments, use2To32GArea);
				Trc_PRT_vmem_reservePagesAboveBar_allocate_large_pages(numSegments, use2To32GArea);
				ptr = j9allocate_1M_fixed_pages(numSegments, use2To32GArea, ttkn);

				LP_DEBUG_PRINTF3("\t j9allocate_1M_fixed_pages(0x%zx, 0x%x) returned 0x%zx\n", \
								 numSegments, use2To32GArea, ptr);
				if (NULL == ptr) {
					if (TRUE == useStrictPageSize) {
						goto _end;
					} else {
			...
			if ((NULL == ptr) &&
				...
				LP_DEBUG_PRINTF2("\t reservePagesAboveBar calling j9allocate_1M_pageable_pages_above_bar(0x%zx, 0x%x)\n", \
								 numSegments, use2To32GArea);
				Trc_PRT_vmem_reservePagesAboveBar_allocate_large_pageable_pages_above_bar(numSegments, use2To32GArea);
				ptr = j9allocate_1M_pageable_pages_above_bar(numSegments, use2To32GArea, ttkn);

				LP_DEBUG_PRINTF3("\t j9allocate_1M_pageable_pages_above_bar(0x%zx, 0x%x) returned 0x%zx\n", \
								 numSegments, use2To32GArea, ptr);
				if (NULL == ptr) {
					if (TRUE == useStrictPageSize) {
						goto _end;
			...
			LP_DEBUG_PRINTF4("\t reservePagesAboveBar invalid params: byteAmount = 0x%x, pageSize = 0x%x, pageFlags = 0x%x, use2To32GArea = 0x%x\n", \
							 byteAmount, pageSize, pageFlags, use2To32GArea);
		}
	}
...
default_pageSize_reserve_memory(struct OMRPortLibrary *portLibrary, uintptr_t byteAmount, struct J9PortVmemIdentifier *identifier, uintptr_t mode, OMRMemCategory *category)
{
	...
	if (J9_ARE_ANY_BITS_SET(OMRPORT_VMEM_MEMORY_MODE_VIRTUAL, mode)) {
...
#if 0
		/* j9allocate_4K_pages_above_bar() uses IARV64 to reserve memory.
		 ...
		 */
		ptr = j9allocate_4K_pages_above_bar(allocSize);
#else /* if 0 */
		{
			__mopl_t mymopl;
			int32_t rc;
...
#if defined(OMR_ENV_DATA64)
	/* Invalid input */
	if (use2To32GArea && !PPG_twoToThirtyTwoGigAreaSupported) {
...
getPageInfo(struct OMRPortLibrary *portLibrary)
{
	...
	uintptr_t twoToThirtyTwoGigAreaSupported = 0;
	...
	/* check if RCEUSE2GTo32GAREAOK bit in RCE is set */
	PPG_twoToThirtyTwoGigAreaSupported = FALSE;
	twoToThirtyTwoGigAreaSupported = Get_Two_to_Thirtytwo_Supported();

	LP_DEBUG_PRINTF1("\nGet_Two_to_Thirtytwo_Supported returned : %i\n", twoToThirtyTwoGigAreaSupported);

	if (1 == twoToThirtyTwoGigAreaSupported) {
		PPG_twoToThirtyTwoGigAreaSupported = TRUE;
	}
