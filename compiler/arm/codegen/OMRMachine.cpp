...
TR::RealRegister *OMR::ARM::Machine::freeBestRegister(TR::Instruction     *currentInstruction,
   ...
   switch (rk)
      {
      case TR_GPR:
         generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::ARMOp_ldr, currentNode, best, tmemref, currentInstruction);
         break;

      case TR_FPR:
         //if targetRegister is FS0-FS15, using TR::InstOpCode::ARMOp_flds.
         generateTrg1MemInstruction(self()->cg(), (isSinglePrecision ? TR::InstOpCode::ARMOp_flds : TR::InstOpCode::ARMOp_fldd), currentNode, best, tmemref, currentInstruction);
         break;
...
TR::RealRegister *OMR::ARM::Machine::reverseSpillState(TR::Instruction      *currentInstruction,
   ...
   switch (rk)
      {
      case TR_GPR:
         generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::ARMOp_str, currentNode, tmemref, targetRegister, currentInstruction);
         break;
      case TR_FPR:
            //if targetRegister is FS0-FS15, using TR::InstOpCode::ARMOp_fsts.
            generateMemSrc1Instruction(self()->cg(), (isSinglePrecision ? TR::InstOpCode::ARMOp_fsts : TR::InstOpCode::ARMOp_fstd), currentNode, tmemref, targetRegister, currentInstruction);
         break;
      }
   return targetRegister;
   }
...
static void registerCopy(TR::Instruction     *precedingInstruction,
   ...
   switch (rk)
      {
      case TR_GPR:
         new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(precedingInstruction, TR::InstOpCode::ARMOp_mov, node, targetReg, sourceReg, cg);
         break;
      case TR_FPR:
         ...
         if (isTargetSinglePrecision)
            ...
            if (isSourceSinglePrecision)
               {
                new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(precedingInstruction, TR::InstOpCode::ARMOp_fcpys, node, targetReg, sourceReg, cg);
               }
            else
               {
               generateTrg1Src1Instruction(cg, TR::InstOpCode::ARMOp_fcvtsd, node, targetReg, sourceReg);
               }
            }
         else
            {
            if (isSourceSinglePrecision)
               {
               generateTrg1Src1Instruction(cg, TR::InstOpCode::ARMOp_fcvtds, node, targetReg, sourceReg);
               }
            else
               {
               new (cg->trHeapMemory()) TR::ARMTrg1Src1Instruction(precedingInstruction, TR::InstOpCode::ARMOp_fcpyd, node, targetReg, sourceReg, cg);
               }
           }
         break;
      }
...
static void registerExchange(TR::Instruction     *precedingInstruction,
   ...
   if (rk != TR_GPR)
      ...
      {
      new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(precedingInstruction, TR::InstOpCode::ARMOp_eor, node, targetReg, targetReg, sourceReg, cg);
      new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(precedingInstruction, TR::InstOpCode::ARMOp_eor, node, sourceReg, targetReg, sourceReg, cg);
      new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(precedingInstruction, TR::InstOpCode::ARMOp_eor, node, targetReg, targetReg, sourceReg, cg);
      }
   }
