...
extern "C" void _patchVirtualGuard(uint8_t* locationAddr, uint8_t* destinationAddr, int32_t smpFlag)
   ...
   if (debugTrace)
      {
      printf("####> Patching VGNOP at locationAddr %p (%" OMR_PRIxPTR "), destinationAddr %p (%" OMR_PRIxPTR "), smpFlag: %d\n", volatileLocationAddr, *reinterpret_cast<volatile uintptr_t*>(volatileLocationAddr), destinationAddr, *reinterpret_cast<uintptr_t*>(destinationAddr), smpFlag);
      }
