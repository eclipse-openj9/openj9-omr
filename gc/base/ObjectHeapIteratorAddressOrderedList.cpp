...
{
	if (_objectModel->isSingleSlotDeadObject(_scanPtr)) {
		_deadObjectSize = _objectModel->getSizeInBytesSingleSlotDeadObject(_scanPtr);
	} else {
		_deadObjectSize = _objectModel->getSizeInBytesMultiSlotDeadObject(_scanPtr);
	}
}
...
GC_ObjectHeapIteratorAddressOrderedList::shouldReturnCurrentObject() {
	if (_scanPtr < _scanPtrTop) {
		...
		if (_isDeadObject) {				
			...
			return MM_ForwardedHeader(_scanPtr, _objectModel->compressObjectReferences()).isStrictlyForwardedPointer();
...
GC_ObjectHeapIteratorAddressOrderedList::nextObjectNoAdvance() {
	...
	while (_scanPtr < _scanPtrTop) {						
		...
		if (!_isDeadObject) {
#if defined(OMR_GC_CONCURRENT_SCAVENGER)
			MM_ForwardedHeader header(_scanPtr, _objectModel->compressObjectReferences());
			if (header.isStrictlyForwardedPointer()) {
				uintptr_t sizeInBytesBeforeMove = _objectModel->getConsumedSizeInBytesWithHeaderBeforeMove(header.getForwardedObject());
