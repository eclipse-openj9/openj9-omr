...
1. **Preparing GC Tests** - Currently following 4 tests work are using valgrind requests. They are:
   ...
   So disable other test cases in `GCConfigTest.cpp` in array `gcTests`.  Performace tests in array `perfTests` work fine.

 ## Running Valgrind
 Everything is ready now.

 1. **Running ExampleVM** - run `valgrind --trace-children=yes --leak-check=full ./omrgcexample`. It has infinite allocations, so you need to terminate in manually (`Ctrl + C`) and see the leaks.

2. **Running GC Tests** - run ` valgrind --trace-children=yes --leak-check=full ./omrgctest`.
