      ...
      if (!constantIsImmed10(displacement))
         {
          ...
          if (constantIsImmed8r(foldedOffset, &immValue, &rotate))
             ...
             if (_baseRegister == NULL)
                generateTrg1ImmInstruction(cg, TR::InstOpCode::ARMOp_mov, node, newBase, immValue, rotate);
             else
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ARMOp_add, node, newBase, _baseRegister, immValue, rotate);
             }
          else
             {
             ...
             for (int i = 0; i < 3; i++)
                ...
                if (base != 0)
                   ...
                   if (!isFirstDone)
                      {
                      if (_baseRegister == NULL)
                         generateTrg1ImmInstruction(cg, TR::InstOpCode::ARMOp_mov, node, newBase, base, shift);
                      else
                         generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ARMOp_add, node, newBase, _baseRegister, base, shift);
                      isFirstDone = true;
                      }
                   else
                      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ARMOp_add, node, newBase, newBase, base, shift);
                   }
                }
             }
...
void OMR::ARM::MemoryReference::consolidateRegisters(TR::Register *srcReg, TR::Node *srcTree, bool srcModifiable, TR::CodeGenerator *cg)
   ...
   if (self()->getUnresolvedSnippet() != NULL)
      ...
      if (_indexRegister != NULL)
         ...
         new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(TR::InstOpCode::ARMOp_add, srcTree, tempTargetRegister, _indexRegister, srcReg, cg);
      ...
      if (_indexRegister != NULL)
         ...
         if (self()->isBaseModifiable())
            ...
            tempTargetRegister = cg->allocateRegister();
         new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(TR::InstOpCode::ARMOp_add, srcTree, tempTargetRegister, _baseRegister, _indexRegister, cg);
         ...
         if (self()->isBaseModifiable())
            ...
            tempTargetRegister = cg->allocateRegister();
         new (cg->trHeapMemory()) TR::ARMTrg1Src2Instruction(TR::InstOpCode::ARMOp_add, srcTree, tempTargetRegister, _baseRegister, srcReg, cg);
         if (_baseRegister != tempTargetRegister)
            {
            self()->decNodeReferenceCounts();
            _baseNode = NULL;
   ...
   if (self()->getUnresolvedSnippet())
      ...
      *wcursor = 0xEA000000; // insert a branch to the snippet
      ...
      if (index)
         ...
         *wcursor = preserve;
         if (op == TR::InstOpCode::ARMOp_ldr   || op == TR::InstOpCode::ARMOp_ldrb || op == TR::InstOpCode::ARMOp_str   || op == TR::InstOpCode::ARMOp_strb ||
             op == TR::InstOpCode::ARMOp_ldrsb || op == TR::InstOpCode::ARMOp_ldrh || op == TR::InstOpCode::ARMOp_ldrsh || op == TR::InstOpCode::ARMOp_strh)
            {
            if (base)
               {
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
         else if (op == TR::InstOpCode::ARMOp_flds || op == TR::InstOpCode::ARMOp_fldd || op == TR::InstOpCode::ARMOp_fsts || op == TR::InstOpCode::ARMOp_fstd)
            {
            if (base)
               {
...
#endif
         else if (op == TR::InstOpCode::ARMOp_add)
            {
            if (base)
               {
               base->setRegisterFieldRN(wcursor);
            ...
            {
            TR_ASSERT(op == TR::InstOpCode::ARMOp_mov, "unknown opcode in an unresolved memory reference");
            mBase->setRegisterFieldRM(wcursor);
            }
         wcursor++;
         cursor += ARM_INSTRUCTION_LENGTH;
      ...
      {
      int32_t displacement;
      if (!index && op != TR::InstOpCode::ARMOp_add)               // common encoding for all true load/store instructions with immediate offsets
         {
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
         if (!(op == TR::InstOpCode::ARMOp_flds || op == TR::InstOpCode::ARMOp_fldd || op == TR::InstOpCode::ARMOp_fsts || op == TR::InstOpCode::ARMOp_fstd))
            {
#endif
            base->setRegisterFieldRN(wcursor);
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
      ...
      if (op == TR::InstOpCode::ARMOp_ldr || op == TR::InstOpCode::ARMOp_str || op == TR::InstOpCode::ARMOp_ldrb || op == TR::InstOpCode::ARMOp_strb)
         {
         // addressing mode 2
         if (index)
            {
      ...
      else if (op == TR::InstOpCode::ARMOp_ldrsb || op == TR::InstOpCode::ARMOp_ldrh || op == TR::InstOpCode::ARMOp_ldrsh || op == TR::InstOpCode::ARMOp_strh) // addressing mode 3
         {
         if (index)
            {
            base->setRegisterFieldRN(wcursor);
      ...
      else if (op == TR::InstOpCode::ARMOp_add)                     // op is TR::InstOpCode::ARMOp_add for delayed offsets (see loadAddrEvaluator)
         {
         base->setRegisterFieldRN(wcursor);
      ...
      else if (op == TR::InstOpCode::ARMOp_ldfd || op == TR::InstOpCode::ARMOp_stfd || op == TR::InstOpCode::ARMOp_ldfs || op == TR::InstOpCode::ARMOp_stfs)
         {
...
#endif
      else if (op == TR::InstOpCode::ARMOp_ldrex || op == TR::InstOpCode::ARMOp_strex)
         {
...
uint32_t OMR::ARM::MemoryReference::estimateBinaryLength(TR::InstOpCode::Mnemonic op)
   ...
   else if (op == TR::InstOpCode::ARMOp_ldr || op == TR::InstOpCode::ARMOp_str || op == TR::InstOpCode::ARMOp_ldrb || op == TR::InstOpCode::ARMOp_strb || op == TR::InstOpCode::ARMOp_ldrex || op == TR::InstOpCode::ARMOp_strex)
      {
   ...
   else if ( op == TR::InstOpCode::ARMOp_ldrsb || op == TR::InstOpCode::ARMOp_ldrh || op == TR::InstOpCode::ARMOp_ldrsh || op == TR::InstOpCode::ARMOp_strh )
      {
      if(constantIsUnsignedImmed8(self()->getOffset()))
         {
         return ARM_INSTRUCTION_LENGTH;
...
uint8_t *OMR::ARM::MemoryReference::encodeLargeARMConstant(uint32_t *wcursor, uint8_t *cursor, TR::Instruction *currentInstruction, TR::CodeGenerator *cg)
...
#if (defined(__VFP_FP__) && !defined(__SOFTFP__))
   vfpInstruction = (op == TR::InstOpCode::ARMOp_flds || op == TR::InstOpCode::ARMOp_fldd || op == TR::InstOpCode::ARMOp_fsts || op == TR::InstOpCode::ARMOp_fstd);
#endif

   TR_ASSERT((op == TR::InstOpCode::ARMOp_add || op == TR::InstOpCode::ARMOp_ldr || op == TR::InstOpCode::ARMOp_ldrb || op == TR::InstOpCode::ARMOp_str || op == TR::InstOpCode::ARMOp_strb ||
           op == TR::InstOpCode::ARMOp_ldrsb || op == TR::InstOpCode::ARMOp_ldrh || op == TR::InstOpCode::ARMOp_ldrsh || op == TR::InstOpCode::ARMOp_strh || vfpInstruction),
           "unrecognized opcode");
   ...
   if(op == TR::InstOpCode::ARMOp_str || op == TR::InstOpCode::ARMOp_strh || op == TR::InstOpCode::ARMOp_strb || vfpInstruction ||
...
static void loadRelocatableConstant(TR::Node               *node,
   ...
   if (symbol->isStartPC())
      ...
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ARMOp_sub, node, reg, reg, 0xad, 16);
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ARMOp_sub, node, reg, reg, 0xbe, 8);
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::ARMOp_sub, node, reg, reg, 0xef, 0);
      return;
      }
