        ...
        for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
            ...
            if ((nextIndVar->getSymRef()->getSymbol()->getDataType()
                ...
                if (performElimination
                    ...
                    if (!disableIVEPostDominatorsCheck) {
                        if (trace())
                            comp()->getLogger()->prints("Checking post dominator relationship between primary and "
                                                        "derived induction variable increments");
...
void TR_LoopCanonicalizer::rewritePostToPreIncrementTestInBlock(TR::Block * const block)
{
    ...
    if (left != updateBase) {
        if (trace()) {
            comp()->getLogger()->printf("Post- to pre-increment transformation looking for store of #%d "
