...
void *
JitBuilder::Monitor::operator new(size_t size)
   {
   return TR::globalAllocator().allocate(size);
   }

void
JitBuilder::Monitor::operator delete(void *p)
   {
   TR::globalAllocator().deallocate(p, sizeof(TR::Monitor));
   }

TR::Monitor *
JitBuilder::Monitor::create(char *name)
   {
   return new TR::Monitor(name);
   }

JitBuilder::Monitor::Monitor(char const *name) :
      _name(name)
   {
