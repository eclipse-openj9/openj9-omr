...
void TR::SoundnessRule::checkNodeSoundness(TR::TreeTop *location, TR::Node *node, TR::NodeChecklist &ancestorNodes,
    TR::NodeChecklist &visitedNodes)
{
    ...
    checkSoundnessCondition(location, !ancestorNodes.contains(node), "n%dn must not be its own ancestor",
        node->getGlobalIndex());
...
void TR::ValidateLivenessBoundaries::updateNodeState(TR::Node *node, TR::NodeSideTable<TR::NodeState> &nodeStates,
    ...
    if (TR::isILValidationLoggingEnabled(comp())) {
        if (!liveNodes.isEmpty()) {
            ...
            for (TR::LiveNodeWindow::Iterator lnwi(liveNodes); lnwi.currentNode(); ++lnwi) {
                log->printf("%sn%dn", separator, lnwi.currentNode()->getGlobalIndex());
                separator = ", ";
            }
            log->prints("}\n");
        }
