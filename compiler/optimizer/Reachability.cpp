    ...
    blocknum_t *stack, blocknum_t *depthMap, TR_BitVector *result)
{
    ...
    if (result->isSet(inputBlockNum)) {
        logprintf(trace, log, "    Propagate block_%d to block_%d\n", blockNum, inputBlockNum);
        result->set(blockNum);
    } else {
        logprintf(trace, log, "    No change to block_%d from block_%d\n", blockNum, inputBlockNum);
    }
}

void TR_ReachabilityAnalysis::traverse(blocknum_t blockNum, int32_t depth, blocknum_t *stack, blocknum_t *depthMap,
    TR_BitVector *result)
{
    //
    ...
    logprintf(trace, log, "  traverse %sblock_%d depth %d\n", value ? "origin " : "", blockNum, depth);
    ...
    if (depthMap[blockNum] == depth) {
        while (1) {
            ...
            if (top == blockNum) {
                ...
                logprintf(trace, log, "    Loop: propagate block_%d to block_%d\n", blockNum, top);
...
void TR_ForwardReachability::propagateInputs(blocknum_t blockNum, int32_t depth, blocknum_t *stack,
...
void TR_ForwardReachabilityWithoutExceptionEdges::propagateInputs(blocknum_t blockNum, int32_t depth, blocknum_t *stack,
    blocknum_t *depth_map, TR_BitVector *closure)
{
    TR::Block *block = getBlock(blockNum);
...
void TR_BackwardReachability::propagateInputs(blocknum_t blockNum, int32_t depth, blocknum_t *stack,
    blocknum_t *depth_map, TR_BitVector *closure)
{
    TR::Block *block = getBlock(blockNum);
