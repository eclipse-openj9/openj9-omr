...
TR::Instruction *generateAlignmentNopInstruction(TR::CodeGenerator *cg, TR::Node *n, uint32_t alignment,
    TR::Instruction *preced)
{
    ...
    TR::Register *srcReg, int32_t shiftAmount, TR::Instruction *preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory()) TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rlwinm, n, trgReg, srcReg,
            shiftAmount % 32, temp, preced, cg);
    return new (cg->trHeapMemory())
    ...
    TR::Register *srcReg, int32_t shiftAmount, TR::Instruction *preced)
{
    ...
    if (preced)
        return new (cg->trHeapMemory()) TR::PPCTrg1Src1Imm2Instruction(TR::InstOpCode::rldicr, n, trgReg, srcReg,
            shiftAmount % 64, mask, preced, cg);
    return new (cg->trHeapMemory())
...
void generateZeroExtendInstruction(TR::Node *node, TR::Register *trgReg, TR::Register *srcReg, int32_t bitsInTarget,
    TR::CodeGenerator *cg)
{
